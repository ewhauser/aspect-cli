...
def _tree_sitter_impl(ctx):
    # Define repositories for each grammar
    http_archive(
        name = "tree_sitter_go",
        ...
        build_file_content = _DEFAULT_BUILD,
    )
...
tree_sitter = module_extension(
    implementation = _tree_sitter_impl,
)
